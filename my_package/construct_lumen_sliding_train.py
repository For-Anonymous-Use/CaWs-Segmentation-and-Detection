...
def sliding_proc(cta_arr, seg_arr, stride, pat_name, o, s, d, dir_nnunet):
    if seg_arr.any():
        lumen_voxel_per_slice = seg_arr.sum(axis=(1, 2))
        z_max = seg_arr.shape[0]
        cum_sum = np.cumsum(lumen_voxel_per_slice)
        print(cum_sum, pat_name)
        upper = 160
        lower = 0  # 因为区间是左闭右开的区间
        count = 1
        while True:
...
def web_trian_gen(raw_cta, raw_seg, dir_nnunet, stride):
    ...
    for cta, seg in zip(ctas, segs):
        ...
        new_cta = cta_arr[79:508]  # 截取脖子
        new_seg = seg_arr[79:508]
        join = os.path.join
        ...
        new_seg[new_seg == 3] = 0
        new_seg[new_seg == 4] = 0
        sliding_proc(new_cta, new_seg, stride, pat_name, o, s, d, dir_nnunet)  # 脖子上滑动构造训练集
    # 生成json文件
...
if __name__ == '__main__':
    ...
    dir_nnunet = "/home/txz/PSI-Seg/nnUNet_raw_data_base/nnUNet_raw_data/Task555_utimate_proc"
...
from batchgenerators.utilities.file_and_folder_operations import *
import os
path = "/home/txz/PSI-Seg/nnUNet_raw_data_base/nnUNet_raw_data/Task555_utimate_proc/imagesTr"
dir = listdir(path)
a = load_pickle("/home/txz/PSI-Seg/nnUNet_preprocessed/Task666_utimate_proc/splits_final.pkl")
b = [0]*3
for fold in range(len(a)):
    b[fold] = OrderedDict([("train", np.array([i[:-12] for i in dir if i[:-15] in a[fold]["train"]])),
                       ("val", np.array([(i[:-12])for i in dir if i[:-15] in a[fold]["val"]]))])
